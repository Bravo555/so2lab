...
lprefix=dane/elements
rprefix=dane/pierwiastki

for file in "$lprefix"/*; do
    [ -L "$file" ] && continue # skip symlinks

    fname=$(basename "$file")
    [ ! -e "$rprefix/$fname" ] && [ ! -L "$rprefix/$fname" ] && echo "P:$fname"
done

for file in "$rprefix"/*; do
    [ -L "$file" ] && continue # skip symlinks

    fname=$(basename "$file")
    [ ! -e "$lprefix/$fname" ] && [ ! -L "$lprefix/$fname" ] && echo "L:$fname"
done
